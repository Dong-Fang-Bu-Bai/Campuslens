...
- ✅ **职责清晰**: DINOv2 使用 GPU 推理，FAISS 保持 CPU 检索
...
macOS 不支持 CUDA，使用 CPU requirements。所有平台均保留 `faiss-cpu==1.7.4`。
...
pip install -r requirements-cpu.txt -r requirements-test.txt
```
