...
def check_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[ERROR] 服务异常，状态码: {response.status_code}")
            return False
    except requests.exceptions.ConnectionError:
        print("[ERROR] 无法连接到服务，请确认服务已启动")
...
def check_index():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[ERROR] 获取索引状态失败，状态码: {response.status_code}")
            return False
    except Exception as e:
...
def main():
    ...
    if health_ok:
        ...
        if health_ok and index_ok:
            print("[OK] 所有检查通过，服务可以正常使用")
        else:
            print("[WARN] 服务运行正常，但需要重建索引")
        print("=" * 60)
